...
export class DatabasesService {
  ...
  async create(data: { name: string; type: string; projectId: number }) {
    ...
    // Bước 4: Lưu thông tin khởi tạo Database vào bảng DatabaseInstance với trạng thái 'provisioning' (đang chuẩn bị)
    ...
    // Bước 5: Khởi chạy quá trình tạo Database ngầm (chạy nền) để không làm treo giao diện người dùng
  ...
  /**
   * Hàm chạy ngầm (Background Task) để tạo và chạy Docker Container cho Database.
   * Giải thích: Hàm này làm việc nặng nhọc như tải Image từ DockerHub, map Port, và mã hóa chuỗi kết nối.
   */
  private async provisionDatabaseBackground(
    ...
    try {
      // 1. Tải (Pull) Docker Image tương ứng (VD: postgres:alpine) từ Docker Hub về máy chủ
      try {
